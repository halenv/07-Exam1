...
def problem3(rect, n, window):
    ...
    window.render()
    center = rg.Point(rect.get_lower_right_corner().x, rect.get_lower_right_corner().y)
    radius = rect.get_height() / 2
    for k in range(n):
        circle = rg.Circle(center, radius)
        circle.attach_to(window)
        center.x = center.x + ((2 * radius) * math.sin(45))
        center.y = center.y + ((2 * radius) * math.cos(45))
    window.render()
